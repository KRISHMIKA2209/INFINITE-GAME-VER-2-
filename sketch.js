...
function preload()
{
  ...
  jungle_img=loadImage("jungle3.jpg");
...
function draw()
{
  // increase moknkey sprite when score increase in skipe count of 10
    switch(score){
  ...
  if(gameState===START)
  {
   ...
   text("4.Try to collect max oranges to get 2X survival time",50,190);
   ...
   text("7.Avoid the obstacles otherwise you will lose 1 LIFE from 5",50,280);
  ...
  {
    imageMode(CENTER);
    image(jungle_img,width/2,height/2,width,height);
